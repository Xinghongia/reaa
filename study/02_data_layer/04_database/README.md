...
1. **实体级**: 在实体类上加 `@TypeConverters(Converters::class)`
2. **数据库级**: 在 AppDatabase 上加 `@TypeConverters(Converters::class)`

**原理**:
```
实体字段类型 → TypeConverter → SQLite 支持的类型
Date        → dateToTimestamp → Long
List<String> → fromStringList → String (逗号分隔)
```
